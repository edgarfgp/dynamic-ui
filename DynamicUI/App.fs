...
module App =
    ...
    let handleHomeExternalMsg externalMsg =
        ...
        | HomePage.ExternalMsg.NavigateToDetail music -> Cmd.ofMsg (GoToDetailPage music)

    let navigationMapper model =
        let detailModel = model.DetailPageModel
        match detailModel with
        | None -> model
        | Some _ -> { model with DetailPageModel = None }
    ...
    let update msg model =
        ...
        | HomePageMsg msg ->
            ...
            { model with HomePageModel = hModel },
            Cmd.batch [ Cmd.map HomePageMsg cmd
                        externalHomeMsg ]

        | DetailPageMsg _ -> { model with DetailPageModel = None }, Cmd.none

        | GoToDetailPage music ->
            let dModel = DetailPage.init music
            { model with
                  DetailPageModel = Some dModel },
            Cmd.none
